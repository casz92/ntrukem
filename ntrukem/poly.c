...
void poly_S3_sample(poly *r, const unsigned char *seed, const unsigned char nonce, int size)
{
  uint32_t buf[(NTRU_N+7)/8];
  unsigned char extseed[size+8];
  int i;
...
void poly_S3_sample_plus(poly *r, const unsigned char *seed, const unsigned char nonce, int size)
{
  ...
  poly_S3_sample(r, seed, nonce, size);
...
void poly_Rq_getnoise(poly *r, const unsigned char *seed, const unsigned char nonce)
{
  ...
  poly_S3_sample(r, seed, nonce, NTRU_SEEDBYTES);
  for(i=0; i<NTRU_N; i++)
